...
class match_mapper_db extends match_mapper{
    ...
    public function load($match_id = null) {
        ...
        $query_for_slots = 'SELECT m.*, p.account_id, CASE WHEN u.account_id IS NULL THEN p.personaname ELSE u.personaname END AS personaname, CASE WHEN u.account_id IS NULL THEN p.steamid ELSE u.steamid END AS steamid, CASE WHEN u.account_id IS NULL THEN \'false\' ELSE \'true\' END as isUser FROM '.db::real_tablename('slots').' as m, '.db::real_tablename('players').' as p LEFT JOIN '.db::real_tablename('users').' as u ON p.account_id=u.account_id WHERE m.match_id=? AND p.account_id=m.account_id';
        //'SELECT * FROM '.db::real_tablename('slots').' as m, '.db::real_tablename('players').' as p WHERE m.match_id=? AND p.account_id=m.account_id';
    ...
    public function insert(match $match) {
        ...
        foreach($slots as $slot) {
            if ($slot->get('account_id') != player::ANONYMOUS) {
                /*$db->insert_pdo(db::real_tablename('players'), array(
    ...
    public function update($match, $lazy = true) {
        ...
        foreach($slots as $slot) {
            // update accounts
            $db->update_pdo(db::real_tablename('players'), array(
                'account_id' => $slot->get('account_id'),
                'steamid' => player::convert_id($slot->get('account_id'))
