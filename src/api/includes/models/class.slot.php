...
class slot extends stat_object {
    ...
    /**
     * steam_id - the player's 64-bit Steam ID - will be set to "-1" if the player has set their account to private.
     * @var int
     */
    protected $_steamid;
    /**
     * personaname - the player's personaname.
     * @var string
     */
    ...
    // hack to ignore the fields I added when saving
    public function get_data_array($save = true) {
        $data = get_object_vars($this);
        $ret = array();
        foreach($data as $key => $value) {
            if (!is_array($value)) {
                if (!in_array($key, array('_steamid', '_personaname', '_isUser')) || !$save) {
